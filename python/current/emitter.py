...
class Emitter(pygame.sprite.Sprite):
    ...
    def __init__(self, synch_cycle, emit_sound, play_duration, location, position, speed):
        ...
        self.rect = pygame.Rect(position, (GRID_SIZE, GRID_SIZE))
    ...
    def update(self, cycle):
        ...
        moveto = "moveto"
        if (self.rect.centerx % GRID_SIZE == int(GRID_SIZE/2) + 1 and 
            self.rect.centery % GRID_SIZE == int(GRID_SIZE/2) + 1):
            moveto = "MOVETO"

        timeline_logger.log(f"em{self.id}: {moveto}: {self.rect.center}", cycle)
        
        # different tile?
        new_location = pos_to_loc(self.rect.center)
        if self.location != new_location:
            self.location = new_location

        # is direction changing?
        new_direction = self.get_direction(exit_info)
    ...
    def get_direction(self, exit_info):
        ...
        current_tile_centre = g.get_tile_rect(self.location).center
